...
let RegisterPage = class RegisterPage {
    constructor(router, fb, emailComposer, facebook, db, fireauth, toastController) {
        this.router = router;
        this.fb = fb;
    ...
    loginF() {
        return this.facebook.login(['email'])
            .then(response => {
            const facebookCredential = firebase__WEBPACK_IMPORTED_MODULE_9__["auth"].FacebookAuthProvider
    ...
    goIn() {
        if (this.hasVerifiedEmail) {
            ...
            }).then(newUser => { console.log("success"); }, error => { console.log("error"); });
            this.signO = true;
        }
        else {
